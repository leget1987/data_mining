...
class GbBlogParse:
    ...
    def parse_post(self, response: requests.Response):
        ...
        # обработка ошибки, если в посте нет картинки
        try:
            first_image_url = soup.find('div', attrs={'class': "blogpost-content content_text content js-mediator-article"}).select_one('img').get('src')
        ...
        data = {
            "post_data": {
                ...
                # такой вариант добавлял бы и ссылку на видео, но иногда регулярка находила чуть больше данных,
                # чем просто ссылка. Скорее всего из за ошибок в верстке
                # 'first_image_url': re.findall('src=\"(.+)\"', soup.find("div", attrs={
                #     "class": "blogpost-content content_text content js-mediator-article"}).attrs.get('content'))[0],
                'date_public': datetime.fromisoformat(soup.find('time').attrs.get('datetime'))
            },
            "author_data": {
